...
Every notebook here is executed nightly by
[`../scripts/run_examples.py`](../scripts/run_examples.py), which then
photographs the widget each one built for the top-level README. Run it yourself
with `python scripts/run_examples.py`, or `python scripts/run_examples.py 06` to
iterate on one — the whole corpus takes about 90 seconds, since these are
network bound rather than compute bound.
